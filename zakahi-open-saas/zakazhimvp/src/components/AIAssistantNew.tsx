...
export default function AIAssistantNew({ menuItems }: AIAssistantNewProps) {
  ...
  const generateResponse = (userMessage: string): string => {
    ...
    if (lowerMessage.includes('традиционн')) {
      const traditionalItems = menuItems.filter(item => 
        item.tags.includes('традиционное')
      );
      if (traditionalItems.length > 0) {
        return `Традиционные греческие блюда: ${traditionalItems.map(i => i.name).slice(0, 5).join(', ')}. Могу рассказать подробнее о любом из них!`;
      }
      return 'У нас много традиционных греческих блюд! Могу порекомендовать Тиропита, Спанакопита, Гирос или Сувлаки. Что вас интересует?';
    }
    ...
    >
      ...
      <div className="px-4 pb-3">
        <div className="flex gap-2 overflow-x-auto scrollbar-hide">
          ...
          <motion.button
            onClick={() => setInputValue('Какие традиционные блюда у вас есть?')}
            className="px-4 py-2 bg-white rounded-full text-sm whitespace-nowrap shadow-sm hover:shadow-md transition-all"
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
          >
